    ...
    def make_token(self, token_type="access"):
        ...
        if token_type == "access":
            return create_access_token(
                ...
                expires_delta=timedelta(days=10)  # match JWT_ACCESS_TOKEN_EXPIRES in config
            )
...
# Custom error response for invalid/malformed token
# IMPORTANT: This fires on optional=True routes when a stale/bad token is present in headers.
# Without this, Flask-JWT returns its own 422 or 401 before the route body runs.
@jwt.invalid_token_loader
def invalid_token_callback(error):
    return error_response(f'Invalid token: {error}', status_code=401)
